...
export default function (pi: ExtensionAPI): void {
	...
	const deliveredIds = (): Set<string> => readPersistedMailIds(uiCtx?.sessionManager.getSessionFile(), "subagents-mail");
	const pump = createWakePump({
		...
		inject: (digest, envelopeIds) => {
			pi.sendMessage({ content: digest, customType: "subagents-mail", display: true, details: { envelopeIds } }, WAKE_DELIVERY);
		},
	});
