/**
 * mail/envelope.ts — the one envelope shape for every message, the address
 * grammar, and a dependency-free monotonic ULID generator. Envelope ids double
 * as mailbox filenames and define processing order, so ids must sort by
 * creation time.
 *
 * Hub-and-spoke: only three envelope types exist — `message` (main/user →
 * agent), `report` (agent → main), `error` (runtime/system → main or agent).
 * There are no questions, answers, or escalations.
 *
...
/** ID segments MAY be "main" (default id for singleton types: `scout/main`). */
...
export const ENVELOPE_TYPES = ["message", "report", "error"] as const;
export type EnvelopeType = (typeof ENVELOPE_TYPES)[number];

export interface EnvelopePayload {
	/** Always present: the human-readable content. */
	text: string;
	/** Structured extras (e.g. a report's structured result). */
	data?: unknown;
	/** Persisted completion signal; valid only on report envelopes. */
	final?: boolean;
	/** Task anchors made terminal by this final report or runtime error. */
...
export interface Envelope {
	...
	to: string;
	type: EnvelopeType;
	/** Links report→task anchor; null otherwise. */
	correlationId: string | null;
	/** Causal-chain depth: parent.hops + 1 for message-triggered sends; fresh = 0. */
	hops: number;
...
export function makeEnvelope(options: MakeEnvelopeOptions): Envelope {
	...
	const envelope: Envelope = {
		...
		payload: {
			...
			...(options.terminalAnchors !== undefined ? { terminalAnchors: options.terminalAnchors } : {}),
		},
...
/**
 * The task anchors a terminal envelope (final report / error) makes terminal.
 *
 *   { kind: "anchors" }               — an explicit (possibly empty) anchor list,
 *                                       with correlationId fallback for
 *                                       pre-migration final reports.
 *   { kind: "legacy-unscoped-error" } — an error persisted before terminalAnchors
 *                                       existed; its historical behavior closed
 *                                       ALL tasks for the sender.
 *
 * Callers choose how to honor "legacy-unscoped-error": the await loop treats it
 * as matching any target, unlike an explicit empty anchor list (which matches no
 * targets), while the main-mail commit closes every open task for the sender.
 */
export type TerminalAnchors =
	| { kind: "anchors"; anchors: string[] }
	| { kind: "legacy-unscoped-error" };

export function terminalTaskAnchors(envelope: Envelope): TerminalAnchors {
	if (envelope.payload.terminalAnchors !== undefined) return { kind: "anchors", anchors: envelope.payload.terminalAnchors };
	if (envelope.type === "report" && envelope.payload.final === true && envelope.correlationId !== null) {
		return { kind: "anchors", anchors: [envelope.correlationId] };
	}
	return envelope.type === "error" ? { kind: "legacy-unscoped-error" } : { kind: "anchors", anchors: [] };
}
...
export function validateEnvelope(value: unknown): string[] {
	...
	if (typeof payload !== "object" || payload === null || Array.isArray(payload)) {
		...
		if (p.terminalAnchors !== undefined) {
			if (!Array.isArray(p.terminalAnchors) || !p.terminalAnchors.every((anchor) => typeof anchor === "string" && ENVELOPE_ID_RE.test(anchor))) {
				errors.push("payload.terminalAnchors must be an array of task envelope ids when present");
			}
			if (env.type !== "error" && !(env.type === "report" && p.final === true)) {
				errors.push("payload.terminalAnchors is valid only on final reports or error envelopes");
			}
		}
	}
