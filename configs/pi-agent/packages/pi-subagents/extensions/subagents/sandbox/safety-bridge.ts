 ...
 * delegated to pi-safety over `pi.events`. PURE of pi-safety internals:
 * no import — a runtime rendezvous, so dropping pi-safety can't break loading.
 *
 * Protocol (the pi.events synchronous-claim idiom):
 *   requester: pi.events.emit("subagents:confirm-request", { method:"confirm", claim })
 ...
 * identical to pi-teams' channel, so pi-safety serves both with one handler.
 */

import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

export const CONFIRM_CHANNEL = "subagents:confirm-request";
