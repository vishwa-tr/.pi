 ...
 * `tools` allowlist, wrapping the mutating tools with the guards:
 *
 ...
 *
 * There is NO per-type path territory and NO bash classifier of our own — a
...
export interface SandboxPorts {
	...
	/** Human confirmation via pi-safety; already bound to the agent address. */
...
function summarize(text: string, max = 200): string {
	return ellipsize(text.replace(/\s+/g, " ").trim(), max);
}

function hardDeny(tool: string, reason: string): never {
	throw new Error(
		`Blocked by the subagents sandbox: ${reason}. This is a hard denial (${tool}) — do not retry; work within your scope or report the limitation to the main agent.`,
	);
}
...
function wrapBashTool(tool: ToolDefinition, ports: SandboxPorts): ToolDefinition {
	...
	return {
		...
		async execute(toolCallId, params, signal, onUpdate, ctx) {
			...
			const result = await ports.confirm({ tool: "bash", command: summarize(command, 4096) });
