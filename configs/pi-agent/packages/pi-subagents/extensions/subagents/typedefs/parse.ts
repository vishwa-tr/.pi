 ...
 * YAML frontmatter between `---` fences; the body is the role prose.
 *
 ...
 * duplicate keys are hard errors with line numbers.
 *
 * Schema: name, description, model, thinking, projectContext, tools.
 *
 * TOLERANT to foreign keys: an UNKNOWN top-level field is a WARNING, not an
 * error — a shared definition with a Teams-only field such as `peers: false`
 * also works in Subagents, with that field ignored. Malformed known fields are
 * still hard errors (a capability grant must never be silently mis-parsed).
 *
 * Pure module: no fs.
 */
...
export type ParseResult =
	| { ok: true; definition: TypeDefinition; warnings: string[] }
	| { ok: false; errors: string[] };

const KNOWN_FIELDS = new Set(["name", "description", "model", "thinking", "projectContext", "tools"]);
...
function parseYamlSubset(frontmatter: string): { map: Map<string, RawValue> } | { errors: string[] } {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if ("error" in result) {
			// An unknown key with an unparseable value is still tolerable — we were
			// going to ignore its value anyway. Known keys must parse.
			if (!KNOWN_FIELDS.has(key)) {
				map.set(key, `(unparsed foreign value)`);
				continue;
			}
...
/** Validate the parsed map against the schema. Unknown fields warn, never error. */
function validateConfig(
	map: Map<string, RawValue>,
	filenameStem: string,
): { config: TypeConfig; warnings: string[] } | { errors: string[] } {
	const errors: string[] = [];
	const warnings: string[] = [];
	for (const key of map.keys()) {
		if (!KNOWN_FIELDS.has(key)) warnings.push(`unknown field ${JSON.stringify(key)} ignored (recognized: ${[...KNOWN_FIELDS].join(", ")})`);
	}
...
export function parseTypeFile(content: string, filenameStem: string): ParseResult {
	...
	return { ok: true, definition: { config: validated.config, body: split.body.trim() }, warnings: validated.warnings };
}
