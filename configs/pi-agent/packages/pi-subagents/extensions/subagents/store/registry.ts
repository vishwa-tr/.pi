/**
 * store/registry.ts — the roster: who exists, their state, and live vitals.
 *
 * NO cross-process locking. The single host-scope lease (store/host-lease.ts)
 * guarantees exactly one owning process per session scope, and the JS event
 * loop serializes mutations within it. So registry writes need only be
 * CRASH-atomic — a tmp-file + rename — not concurrency-safe.
 *
...
export interface AgentRecord {
	...
	id: string;
	lifetime: Lifetime;
	/** Human display name; optional only for registries written before labels existed. */
	label?: string;
	/** Incarnation fence `gen_<32hex>` — bumped when the session handle is rebuilt. */
	generationId: string;
	/** sha256 of the resolved constitution (.md) at last wake (live-resolve fence). */
	typeFileHash: string;
	createdAt: string;
	lastActiveAt: string;
	vitals: AgentVitals;
...
/** Get-or-create: existing address is returned as-is (created:false). */
export function upsertAgent(registry: Registry, input: UpsertInput): { record: AgentRecord; created: boolean } {
	...
	if (existing) {
		...
		if (existing.label === undefined && input.label !== undefined) existing.label = input.label;
...
function hasValidIdentity(value: unknown): value is AgentRecord {
	...
	if (r.label !== undefined && typeof r.label !== "string") delete r.label;
	if (typeof r.label === "string") {
		r.label = r.label.replace(/\s+/g, " ").trim();
		if (!r.label) delete r.label;
	}
 ...
 * roster entry to reference or GC it. Only a record whose identity itself is
 * unusable (can't be keyed) is discarded.
 */
