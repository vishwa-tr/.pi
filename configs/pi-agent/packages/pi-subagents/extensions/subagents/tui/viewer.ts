/**
 * tui/viewer.ts — full-screen session viewer. Replays a subagent's Pi-native
 * JSONL through Pi's OWN transcript components, with a header strip of live
 * vitals and a superuser input line (Enter = mail at the turn boundary;
 * alt+Enter = steer). A quiet FYI report goes to main on every superuser send
 * (handled in the runtime).
 *
 * Deliberately lean: it re-reads the session file on a poll + on runtime
 * events and rebuilds components. Robust (no torn-line handling), at the cost
...
function buildComponents(sessionFile: string | null, tui: TUI, cwd: string): Component[] {
	...
	for (const raw of parseSessionEntries(content)) {
		...
		if (message.role === "user") {
			components.push(new UserMessageComponent(flattenTextOnly(message.content)));
...
export function createViewer(options: ViewerOptions): ViewerComponent {
	...
	const reload = (): void => {
		void core.peek(address, 500).then((detail) => {
			...
			const pct = v.ctxPercent !== null ? `ctx ${Math.round(v.ctxPercent)}%` : "ctx ?";
			header = detail.label || detail.address;
			headerMeta = ` · ${detail.state} · ${pct} · ${formatTokens(v.tokens)} tok · ${v.turns} turns`;
			tui.requestRender();
		}).catch(() => {});
	};
