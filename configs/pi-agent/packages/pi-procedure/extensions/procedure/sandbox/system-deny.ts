 ...
 * (Copied verbatim from pi-subagents; only the protected-dir set differs —
 * pi-procedure protects the run store and the procedure library dirs, else an
 * agent could rewrite journals or procedure definitions.)
 *
 ...
 * (macOS/Windows) so a re-cased spelling can't evade the check.
 */
 ...
 * configured dir, and (when the dir is under $HOME) its `~`-relative spelling. It is
 * NOT a complete bash sandbox (env vars, `cd`, symlinks planted at runtime can still
 * evade a text scan) — but it removes the silent, one-line bypass.
 */
