/**
 * tui/picker.ts — the `/teams` roster selector (D22). Native settings-selector
 ...
 * dim hint footer. Flat roster (no teams, D12) + a collapsed `.archive`
 * section.
 *
 * Keys: ↑↓/jk navigate · Enter view · x interrupt · X retire (y/N) ·
 * S stop all · a toggle archive · q close. Action keys are intercepted
 * before the list so SelectList only ever sees navigation input.
 */
...
import { MAIL_ICON } from "./tree-widget.ts";
...
export function isSelectable(row: PickerRow): boolean {
	return row.kind === "agent" || row.kind === "archived" || row.kind === "archive-header";
}

/** One-line text form of a row — derived from `pickerRowItem` so the two can never drift. */
export function pickerRowText(row: PickerRow): string {
	const item = pickerRowItem(row);
	// The archive-header description is an interaction hint, not row data.
	if (row.kind === "archive-header" || !item.description) return item.label;
...
export function pickerRowItem(row: PickerRow): SelectItem {
	switch (row.kind) {
		case "agent": {
			...
			const pct = entry.vitals.ctxPercent !== null ? ` · ${Math.round(entry.vitals.ctxPercent)}% ctx` : "";
			const badge = row.unread > 0 ? ` · ${MAIL_ICON} ${row.unread}` : "";
			return {
				value: `agent:${entry.address}`,
				label: `${STATE_GLYPHS[entry.state]} ${entry.address}${label}`,
				description: `${entry.state}${pct}${badge}`,
			};
		}
		case "archive-header":
			return {
...
export function createPicker(options: PickerOptions): PickerComponent {
	...
	const selectedRow = (): PickerRow | undefined => {
		...
		return rows.find((row) => pickerRowItem(row).value === value);
	};
	...
	return {
		...
		render(width: number): string[] {
			...
			const lines: string[] = [rule, clip(theme.fg("accent", theme.bold(`Teams (${roster.length})`)))];
			if (list) lines.push(...list.render(width));
			else lines.push(theme.fg("muted", "  no agents — spawn one with team_spawn"));
			if (confirmRetire) lines.push(clip(theme.fg("error", `  Retire ${confirmRetire}? [y/N]`)));
			else lines.push(clip(theme.fg("dim", "  ↑↓ move · Enter view · x interrupt · S stop all · X retire · a archive · q close")));
			lines.push(rule);
			return lines;
		},
		handleInput(data: string) {
			...
			// Stop-all: interrupt every working agent (non-destructive; mail stays
			// pending). Uppercase, like retire — a fleet-wide action shouldn't be a
			// stray lowercase keypress away.
			if (data === "S") {
				return void core.interruptAllWorking().then(reload, reload);
			}
			const row = selectedRow();
			if (row?.kind === "agent" && data === "x") return void core.interrupt(row.entry.address).then(reload).catch(reload);
