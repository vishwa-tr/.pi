/**
 * mail/collect.ts — collect-result schema validation (team_collect). PURE.
 *
 ...
 *
 * "Honest" is load-bearing (v1 findings #9/#10): additionalProperties:false
 * closes the object even when properties is absent/partial; const/enum equality
 * is JSON-semantic (order-independent, recursive), not stringify-based; and a
 * `type` naming an unknown keyword is a schema error, not a silent match.
 */
...
function checkSchemaShape(schema: unknown, path: string, errors: string[]): void {
...
/** Validate a collect result's payload.data against the requested schema. */
...
/** The digest annotation for a collect-result report. */
export function collectResultNote(result: ValidationResult): string {
	return result.valid
		? "[collect result — conforms to the requested schema]"
		: `[collect result — SCHEMA VIOLATIONS: ${result.errors.join("; ")}]`;
}
