/**
 * mail/wake-pump.ts — the main-mail auto-wake policy (D24). PURE of Pi: no
 * ExtensionAPI, no fs, no SDK — just the state machine and an injected port.
 *
 * Why this module exists: the policy (when is the host idle? when may we drain?
 * when do we commit?) used to live inside index.ts's `export default function(pi)`
 * closure, tangled with `pi.on(...)` plumbing. That made it untestable — the e2e
 * harness had to RE-IMPLEMENT the lifecycle to exercise it, so a bug in the real
 * gating could never be caught. Extracting it (humble object) leaves index.ts with
 * only literal wiring and lets the suite drive the REAL logic.
 *
 * The rules it encodes:
 *   - Mail is delivered to the host ONLY while it is idle; mid-turn mail waits for
 *     the turn boundary (D11 — mail never interrupts a running turn).
